...
int main()
{
    const bool fric_switch = true;
    const double dt = 5.0;
    const uword max_num_steps = 20000;
    const double terminal_com_z = 20.001;
    const double mol_init_com_z = 20;
    const bool print_switch = true;
    const bool conv_test = false;

    const int incident = 15;
    const int num_trajs = 1;
    const double mol_init_trans_kinE = 0.02;
    const double mol_init_vib_quanta = 15.0;
    const double mol_init_rot_quanta = 10;
	...
	local_is_scattered[local_traj_idx] = efld.is_scattered;
	local_num_steps[local_traj_idx] = efld.run_time.size();
	local_time_elapsed[local_traj_idx] = efld.curr_time;
	...
	local_final_rot_quanta[local_traj_idx] = sys.diabats.numRotQuanta();
	local_init_vib_quanta[local_traj_idx] = mol_init_vib_quanta;
	local_final_vib_quanta[local_traj_idx] = sys.diabats.numVibQuanta();
	local_minimal_com_z[local_traj_idx] = efld.minimal_z;

	vec v_out = sys.molComVelo();
	local_reflection_angle[local_traj_idx] = acos( v_out(2) / norm(v_out) ) * 180.0 / PI;

	if (num_trajs == 1) {
    	    extract(efld.lat_coor_t, 0).save( dir_name+"efld_lat_coor_x.txt", raw_ascii );
    	    extract(efld.lat_coor_t, 1).save( dir_name+"efld_lat_coor_y.txt", raw_ascii );
    	    extract(efld.lat_coor_t, 2).save( dir_name+"efld_lat_coor_z.txt", raw_ascii );
    	    extract(efld.mol_coor_t, 0).save( dir_name+"efld_mol_coor_x.txt", raw_ascii );
    	    extract(efld.mol_coor_t, 1).save( dir_name+"efld_mol_coor_y.txt", raw_ascii );
    	    extract(efld.mol_coor_t, 2).save( dir_name+"efld_mol_coor_z.txt", raw_ascii );
	    conv_to<vec>::from(efld.run_time).save( dir_name+"efld_run_time.txt", raw_ascii );
	    if (!conv_test) {
		conv_to<vec>::from(efld.totE_t).save( dir_name+"efld_totE.txt", raw_ascii );
		conv_to<vec>::from(efld.run_time).save( dir_name+"efld_time.txt", raw_ascii);
	    } else {
		conv_to<vec>::from(efld.totE_t).save( dir_name+"efld_totE_test.txt", raw_ascii );
		conv_to<vec>::from(efld.run_time).save( dir_name+"efld_time_test.txt", raw_ascii);
	    }
	}
    }
    ...
    if (id == 0) {
	data = zeros(num_trajs, 18);
	data.col(0) = conv_to<vec>::from( Col<int>(is_scattered, num_trajs) );
	data.col(1) = conv_to<vec>::from( Col<int>(num_steps, num_trajs) );
	data.col(2) = vec(time_elapsed, num_trajs);
	data.col(3) = vec(walltime_elapsed, num_trajs);
	data.col(4) = vec(init_trans_kinE, num_trajs);
	data.col(5) = vec(init_rot_quanta, num_trajs);
	data.col(6) = vec(init_vib_quanta, num_trajs);
	data.col(7) = vec(final_trans_kinE, num_trajs);
	data.col(8) = vec(final_rot_quanta, num_trajs);
	data.col(9) = vec(final_vib_quanta, num_trajs);
	data.col(10) = vec(minimal_com_z, num_trajs);
	data.col(11) = vec(reflection_angle, num_trajs);
	data.col(12) = vec(init_com_x, num_trajs);
	data.col(13) = vec(init_com_y, num_trajs);
	data.col(14) = vec(init_com_z, num_trajs);
	data.col(15) = vec(init_com_vx, num_trajs);
	data.col(16) = vec(init_com_vy, num_trajs);
	data.col(17) = vec(init_com_vz, num_trajs);
